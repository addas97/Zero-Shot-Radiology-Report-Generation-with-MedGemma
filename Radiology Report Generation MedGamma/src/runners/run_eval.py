...
def build_report_prompt(row_prompt: str, prompt_cfg: Dict[str, Any]) -> str:
    ...
    system_text = prompt_cfg.get("system", "You are an expert radiologist.")
    user_text   = prompt_cfg.get("user",
                                 "Provide a description of the findings in the radiology image.")
    parts = [system_text, user_text]
    if row_prompt and row_prompt.strip() and row_prompt.strip() != user_text.strip():
...
def run_eval(config: Dict[str, Any]) -> None:
    ...
    for idx, row in tqdm(
        ...
        if len(buffer) == batch_size:
            ...
            image_paths = [str(dataset_dir / r["image"].replace("valid/", "", 1)) for r in rows]
            prompts = [
