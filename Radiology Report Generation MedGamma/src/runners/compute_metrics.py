...
def _compute_prf(gt: pd.Series, pred: pd.Series) -> Dict[str, Optional[float]]:
    """
    Compute binary classification metrics for one label.
    Expected output keys: tp, fp, fn, support, precision, recall, f1
    """

    gt   = _binary_series(gt)
    pred = _binary_series(pred)
...
def _compute_label_metrics(
    ...
    """Aggregate label metrics across diseases.
    Compute:
    - macro_f1 from per-label F1 values (ignore None).
    - micro_precision, micro_recall from total_tp/total_fp/total_fn.
    - micro_f1 from micro precision/recall."""

    # Macro F1: average per-disease F1 (ignore None)
    valid_f1s = [row["f1"] for row in f1_rows if row["f1"] is not None]
    macro_f1  = float(sum(valid_f1s) / len(valid_f1s)) if valid_f1s else None

    # Micro metrics: pool TP/FP/FN across all diseases
    micro_precision = (
        float(total_tp / (total_tp + total_fp))
        if (total_tp + total_fp) > 0 else None
    )
    micro_recall = (
        float(total_tp / (total_tp + total_fn))
        if (total_tp + total_fn) > 0 else None
    )
    if micro_precision is not None and micro_recall is not None:
        denom    = micro_precision + micro_recall
        micro_f1 = float(2 * micro_precision * micro_recall / denom) if denom > 0 else 0.0
    else:
        micro_f1 = None
