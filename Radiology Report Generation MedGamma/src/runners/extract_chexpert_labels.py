...
def build_prompt(report_text: str) -> str:
    ...
    return (
        "Extract CheXpert labels from this radiology report. "
        "Output valid JSON only with keys exactly: "
        f"{labels}. "
        "CRITICAL RULES:\n"
        "- Use 1 ONLY if the finding is explicitly confirmed as PRESENT or POSITIVE.\n"
        "- Use 0 if the finding is absent, negated (e.g. \'no effusion\', "
        "\'without pneumothorax\'), uncertain, or not mentioned.\n"
        "- Negation words like \'no\', \'without\', \'absent\', \'clear of\', "
        "\'no evidence of\' mean use 0.\n"
        "- Do not be fooled by mentions of a finding in a negative context.\n\n"
        "Report:\n"
        f"{report_text}\n\n"
        "JSON:"
    )
...
def load_transformers_model(args: argparse.Namespace):
    ...
    """
    - Load tokenizer from args.model_id, set pad token to be tokenizer.eos_token if missing, and use left padding.
    - Resolve torch dtype via torch_dtype = resolve_dtype(args.dtype, torch)
    - Load AutoModelForCausalLM by utoModelForCausalLM.from_pretrained, set trust_remote_code=True and args.device_map.
    - Call model.eval() and return (model, tokenizer, torch).
    """
...
def batch_extract(
    ...
    if not do_sample:
        # Force greedy-compatible defaults --> avoid sampling-flag warnings
    ...
    """
    - Remove prompt tokens before decoding generated text. the prompte tokens are [:,:prompt_len] of output_ids
    - use tokenizer.batch_decode to produce ids of tokens
    - produce generated_texts with tokenizer.batch_decode(ids)
    """
